...
export function generateAbilityFooter(ability: Ability) {
    let footerList: string[] = [
        ability.rarity ? `${capitalize(ability.rarity)} AA` : "Not an AA",
    ];
    ...
    if (!ability.categories.includes("VISITING"))
        footerList.push("Non-Visiting");
...
export function formatRoleEmbed(_guild: Guild, role: FullRole) {
    ...
    if (role.name === "Nephilim") {
        const desc: string[] = [
            "Role uses unique mechanics, check with the info server for its types.",
            "<#1096236057378422915>",
        ];
    ...
    embed.addFields([
        ...
        ...abilities
            ...
            .map((ability) => {
                ...
                return {
                    name: `**${name} [${
                        charges === -1 ? "inf" : "x" + charges
                    }]**${!ability.showCategories ? "" : ` - ${categories}`}`,
...
export function formatItemEmbed(_guild: Guild, item: Item) {
    ...
    if (footer)
        embed.setFooter({ text: footer, iconURL: iconURL ?? undefined });
...
export function formatStatusEmbed(
    ...
    const inflictions = statusLinks.filter(
        (link) => link.linkType === "INFLICTION"
    );
...
export function formatAbilityEmbed(
    _guild: Guild,
    ability: FullAbility,
    ...
    embed.addFields([
        {
            name: showCharges
                ? `**Effect [${
                      ability.charges === -1 ? "inf" : "x" + ability.charges
                  }]**`
                : "**Effect**",
            value: effect,
        },
...
export function formatPerkEmbed(_guild: Guild, perk: FullPerk) {
    ...
    if (footerList.length > 0) {
        ...
        embed.setFooter({
            ...
            text: footerList.length > 0 ? footerList.join(" · ") : "\u200B",
        });
    }
...
export function formatAbilityChanges(
    guild: Guild,
    ability: Ability & { changes: AbilityChange[] }
) {
    const embed = new EmbedBuilder();
    embed.setTitle(`Upgrades/Downgrades for ${capitalize(ability.name)}`);
    embed.setColor("#CE8964");

    const totalUpgrades = ability.changes.filter(
        (change) => change.changeType === "UPGRADE"
    );
    const totalDowngrades = ability.changes.filter(
        (change) => change.changeType === "DOWNGRADE"
    );
    ...
    select.setPlaceholder(
        "Select an upgrade/downgrade to view the full effect"
    );
    select.addOptions(
        [
...
export type FullInventory = NonNullable<
...
export function formatInventory(inventory: FullInventory) {
    ...
    if (inventory.alignment)
        e.addFields({
            name: "Alignment",
            value: capitalize(inventory.alignment),
            inline: false,
        });
    e.addFields({ name: "Coins", value: coinValue, inline: false });

    const baseAbilities =
        inventory.baseAbility.length > 0
            ? inventory.baseAbility
                  .map((ability) => `- ${ability.name} [${ability.charges}]`)
                  .join("\n")
            : "- None";
    const basePerks =
    ...
    const itemValue =
        inventory.items.length > 0
            ? inventory.items.map((item, index) => `- ${item}`).join("\n")
            : "- None";
    const anyAbilities =
        inventory.anyAbilities.length > 0
            ? inventory.anyAbilities
                  .map(
                      (ability) =>
                          `- ${ability.abilityName} [${ability.charges}]`
                  )
                  .join("\n")
            : "- None";
    ...
    const immunityValue =
        inventory.immunities.length > 0
            ? inventory.immunities
                  .map((immunity) => {
                      let result = `- ${
                          immunity.xShot ? `${immunity.xShot}-shot ` : ""
                      }${immunity.name}`;
                      if (immunity.expiry)
                          result += ` (expires <t:${immunity.expiry}:R>)`;
                      return result;
                  })
                  .join("\n")
            : "- None";
        ...
        inventory.effects.length > 0
            ? inventory.effects
                  .map((effect) => {
                      ...
                      if (effect.expiry)
                          value += ` (expires <t:${effect.expiry}:R>)`;
                      return value;
                  })
                  .join("\n")
            : "- None";
...
export async function fetchAndFormatInventory(
    userId: string
): Promise<FetchAndFormattedInventory> {
...
export function formatActionBacklog(actionBacklog: ActionBacklog) {
    ...
    for (const action of actionBacklog.actions) {
        if (action.isPlainText) {
            ...
            if (action.timestamp)
                value += ` - Submitted for <t:${Math.ceil(
    ...
    row.addComponents(
        new ButtonBuilder()
            .setCustomId(processActionBacklog.getCustomID())
            .setLabel("Submit Action/s")
            .setStyle(ButtonStyle.Secondary)
    );
