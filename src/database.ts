...
export async function getRoles() {
	if (cache.roles.length == 0) {
		...
		const goodRoles = roles.filter((r) => r.alignment == 'GOOD').map((r) => r.name);
		const neutralRoles = roles.filter((r) => r.alignment == 'NEUTRAL').map((r) => r.name);
		const evilRoles = roles.filter((r) => r.alignment == 'EVIL').map((r) => r.name);
