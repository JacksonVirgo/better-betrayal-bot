...
export default new Button("set-action-host-notes")
    ...
    .onExecute(async (i, cache) => {
        ...
        const hostNoteFields = embed.data.fields?.filter(
            (field) => field.name == "Host Notes"
        );
        const hostNote = hostNoteFields?.length
            ? hostNoteFields[0].value
            : null;
