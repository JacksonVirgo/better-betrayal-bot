import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, ColorResolvable, EmbedBuilder } from 'discord.js';
...
export default new Button('process-action-backlog')
	...
	.onExecute(async (i, cache) => {
		...
		if (!backlog) return i.reply({ content: 'No action found', ephemeral: true });
		...
		if (!confessional || confessional.type != ChannelType.GuildText)
			return i.reply({ content: 'Confessional channel not found. Please tell the host/s your actions directly instead.', ephemeral: true });
