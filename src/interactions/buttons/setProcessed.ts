import { ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, EmbedData } from 'discord.js';
...
export default new Button('set-processed')
	.setButton(new ButtonBuilder().setLabel('Set Processed').setStyle(ButtonStyle.Secondary))
	.onExecute(async (i, cache) => {
		...
		const playerEmbed = playerMessage ? new EmbedBuilder(playerMessage.embeds[0].data) : null;
		...
		try {
			if (playerMessage && playerEmbed) await playerMessage.edit({ embeds: [playerEmbed] });
