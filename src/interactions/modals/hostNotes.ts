...
export default new Modal(
    "host-notes-action",
    "Viewing/Editing Host Notes"
).onExecute(async (i: ModalSubmitInteraction, cache) => {
    if (!i.guild) return;
    if (!cache)
        return i.reply({ content: "This modal is invalid", ephemeral: true });
    ...
    try {
        ...
        if (!fields)
            return i.reply({ content: "No fields found", ephemeral: true });
