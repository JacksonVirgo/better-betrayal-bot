...
import {
    getClosestAbilityName,
    getClosestImmunityName,
    getClosestItemName,
    getClosestPerkName,
    getClosestRoleName,
...
const data = new SlashCommandBuilder()
    .setName("inventory")
    .setDescription("Command to manage inventories");
...
const types: APIApplicationCommandOptionChoice<string>[] = additionTypes.map(
    (t) => ({ name: t == "aa" ? "AA" : capitalize(t), value: t })
);
const update: APIApplicationCommandOptionChoice<string>[] = updateTypes.map(
    (t) => ({ name: t == "aa" ? "AA" : capitalize(t), value: t })
);
...
data.addSubcommand((sub) =>
    sub
        ...
        .addChannelOption((opt) =>
            opt
                .setName("channel")
                .setDescription("The channel to create the inventory in")
                .setRequired(true)
        ...
        .addUserOption((opt) =>
            opt
                .setName("user")
                .setDescription("The user to create the inventory for")
                .setRequired(true)
        )
        .addStringOption((opt) =>
            opt
                .setName("role")
                .setDescription(
                    "Default role to create the inventory for, sets appropriate values"
                )
                .setRequired(true)
...
data.addSubcommand((sub) =>
    sub
        ...
        .addUserOption((opt) =>
            opt
                .setName("user")
                .setDescription("The user to delete the inventory for")
                .setRequired(false)
        )
);
...
data.addSubcommand((sub) =>
    sub
        ...
        .addIntegerOption((opt) =>
            opt
                .setName("charges")
                .setDescription("The amount of charges to add")
                .setRequired(false)
        )
        .addIntegerOption((opt) =>
            opt
                .setName("expiry")
                .setDescription(
                    "Discord timestamp for when this expires/runs out"
                )
                .setRequired(false)
        )
        .addIntegerOption((opt) =>
            opt
                .setName("xfold")
                .setDescription("Add this value with X-fold/x-shot etc")
                .setRequired(false)
        )
        .addBooleanOption((opt) =>
            opt
                .setName("toggled")
                .setDescription("[PERK] Start the perk toggled/not-toggled")
                .setRequired(false)
        )
        ...
        .addBooleanOption((opt) =>
            opt
                .setName("spellcheck")
                .setDescription("Whether to spellcheck the value")
                .setRequired(false)
        )
);
...
data.addSubcommand((sub) =>
    sub
        ...
        .addStringOption((opt) =>
            opt
                .setName("value")
                .setDescription("The value to update")
                .setRequired(true)
        )
        .addIntegerOption((opt) =>
            opt
                .setName("charges")
                .setDescription("The amount of charges to change to")
                .setRequired(false)
        )
        .addIntegerOption((opt) =>
            opt
        ...
        .addIntegerOption((opt) =>
            opt
                .setName("xfold")
                .setDescription("Change this value with X-fold/x-shot etc")
                .setRequired(false)
        )
        .addBooleanOption((opt) =>
            opt
                .setName("toggled")
                .setDescription(
                    "[PERK] Change the perk to be toggled/not-toggled"
                )
                .setRequired(false)
        )
        .addBooleanOption((opt) =>
...
data.addSubcommand((sub) =>
    sub
        ...
        .addUserOption((opt) =>
            opt
                .setName("user")
                .setDescription("The user to view the inventory for")
                .setRequired(true)
        )
        .addBooleanOption((opt) =>
            opt
...
data.addSubcommand((sub) =>
    sub
        ...
        .addUserOption((opt) =>
            opt
                .setName("user")
                .setDescription("The user to change the values for")
                .setRequired(true)
        )
        .addIntegerOption((opt) =>
            opt
        ...
        .addIntegerOption((opt) =>
            opt
                .setName("itemlimit")
                .setDescription("The item limit to set")
                .setRequired(false)
        )
        .addIntegerOption((opt) =>
            opt
                .setName("coinbonus")
                .setDescription(
                    "The coin bonus % to set. Percent without the period. 500 = 5%."
                )
                .setRequired(false)
        )
        .addStringOption((opt) =>
            opt
                .setName("alignment")
                .setDescription("Set the alignment of the player")
                .setRequired(false)
                .setChoices(
                    {
                        name: "Good",
...
async function updatePinnedInventory(
    i: ChatInputCommandInteraction,
    inv: UpdatePinnedProps
) {
...
async function createInventory(i: ChatInputCommandInteraction) {
    ...
    if (existingInventory)
        return await i.reply({
            content: "That player already has an inventory",
            ephemeral: true,
        });
            ...
            for (const perkAttach of role.perkAttachments) {
                ...
                try {
                    await prisma.basePerk.create({
                        data: {
                            name: perk.name,
                            toggled: perk.categories.includes("TOGGLABLE")
                                ? false
                                : undefined,
                            inventory: {
                                connect: {
    ...
    if (!refetchInventory)
        return await i.reply({
            content: "Failed to create an inventory for that player",
            ephemeral: true,
        });
...
async function viewInventory(i: ChatInputCommandInteraction) {
    const user = i.options.getUser("user", true);
    const hidden =
        i.options.getBoolean("hidden", false) != false ? true : false;

    const inventory = await getInventory(user.id);
    if (!inventory)
        return await i.reply({
            content: "Failed to get inventory",
            ephemeral: true,
        });
...
async function removeInventory(i: ChatInputCommandInteraction) {
    ...
    try {
        ...
        if (!existing)
            return await i.reply({
                content: "That player does not have an inventory",
                ephemeral: true,
            });
        ...
        console.log(err);
        return await i.reply({
            content: "Failed to delete inventory",
            ephemeral: true,
        });
    }
}
...
async function setMiscData(i: ChatInputCommandInteraction) {
    ...
    const alignment = i.options.getString(
        "alignment",
        false
    ) as Alignment | null;
    ...
    if (coins && addCoins)
        return await i.reply({
            content: "You cannot set and add coins at the same time",
            ephemeral: true,
        });
    if (coins && removeCoins)
        return await i.reply({
            content: "You cannot set and remove coins at the same time",
            ephemeral: true,
        });
    ...
    if (!inventory)
        return await i.reply({
            content: "That user does not have an inventory",
            ephemeral: true,
        });
    ...
    const saved = await prisma.inventory.update({
        ...
        data: {
            coins: coins
                ? coins
                : inventory.coins + finalAddCoins - finalRemoveCoins,
    ...
    const changes = [];
    if (inventory.coins != saved.coins)
        changes.push(`- Coins: ${saved.coins} from ${inventory.coins}`);
    if (inventory.inventorySize != saved.inventorySize)
        changes.push(
            `- Item Limit: ${saved.inventorySize} from ${inventory.inventorySize}`
        );
    if (inventory.coinBonus != saved.coinBonus)
        changes.push(
            `- Coin Bonus: ${saved.coinBonus} (aka ${
                saved.coinBonus / 100
            }%) from ${inventory.coinBonus} (aka ${inventory.coinBonus / 100}%)`
        );
    if (inventory.alignment != saved.alignment)
        changes.push(
            `- Alignment: ${saved.alignment} from ${inventory.alignment}`
        );
...
async function addValue(i: ChatInputCommandInteraction) {
    ...
    const spellcheck =
        i.options.getBoolean("spellcheck", false) != false ? true : false;

    if (!additionTypes.includes(type))
        return await i.reply({ content: "Invalid type", ephemeral: true });
    ...
    try {
        switch (type) {
            case "item":
                let itemName = spellcheck
                    ? await getClosestItemName(value)
                    : value;
                await prisma.inventory.update({
                    where: {
                        channelId: inventory.channelId,
                ...
                break;
            case "aa":
                let aaName = spellcheck
                    ? await getClosestAbilityName(value)
                    : value;
                let AAExists = false;
                ...
                if (AAExists)
                    return await i.editReply({
                        content:
                            "That AA already exists, update to add a charge instead",
                    });
                ...
                changes.push(
                    `Added AA: ${aaName} with ${charges ?? 1} charge/s`
                );
                break;

            case "status":
                let statusName = spellcheck
                    ? await getClosestStatusName(value)
                    : value;
                for (const a of inventory.statuses)
                    if (a.statusName == statusName)
                        return await i.editReply({
                            content:
                                "Status with this name already exists in their inventory. Either remove it, or use a different name",
                        });
                ...
                changes.push(
                    `Added Status: ${
                        ...
                        expiry ? ` that expires <t:${expiry}:R>` : ""
                    }`
                );
                break;
            case "immunity":
                let immunityName = spellcheck
                    ? await getClosestImmunityName(value)
                    : value;
                for (const a of inventory.immunities)
                    if (a.name == immunityName)
                        return await i.editReply({
                            content:
                                "Immunity with this name already exists in their inventory. Either remove it, or use a different name",
                        });
                ...
                changes.push(
                    `Added immunity: \`${
                        ...
                        expiry ? ` which expires <t:${expiry}:R>` : ""
                    }`
                );
                break;
            case "effect":
                ...
                for (const a of inventory.effects)
                    if (a.name == effectName)
                        return await i.editReply({
                            content:
                                "Effect with this name already exists in their inventory. Either remove it, or use a different name",
                        });
                ...
                break;
            case "perk":
                let perkName = spellcheck
                    ? await getClosestPerkName(value)
                    : value;
                for (const perk of inventory.basePerk)
                    if (perk.name == perkName)
                        return await i.editReply({
                            content:
                                "Perk with this name already exists in their inventory. Either remove it, or use a different name",
                        });
                ...
                changes.push(
                    `Added perk: \`${perkName}\`${
                        showToggle
                            ? ` which is ${toggle ? "enabled" : "disabled"}`
                            : ""
                    }`
                );
                break;
            case "ability":
                let abilityName = spellcheck
                    ? await getClosestAbilityName(value)
                    : value;
                for (const ability of inventory.baseAbility)
                    if (ability.name == abilityName)
                        return await i.editReply({
                            content:
                                "Ability with this name already exists in their inventory. Either remove it, or use a different name",
                        });
                ...
                changes.push(
                    `Added ability: \`${abilityName}\`${
                        charges ? ` with ${charges} charge/s` : ""
                    }`
                );
                break;
        }
        ...
        switch (type) {
            case "aa":
                ...
                if (!AAID)
                    return await i.editReply({
                        content:
                            "Cannot update an AA that does not exist, silly",
                    });
                ...
                changes.push(
                    `Changed charges of AA: \`${AAName}\` from ${oldCharges} to ${charges}`
                );
                break;
            case "status":
                let statusName = spellcheck
                ...
                if (!statusId)
                    return await i.editReply({
                        content:
                            "Cannot update a status that does not exist, silly",
                    });
                ...
                changes.push(
                    `Changed Status: \`${statusName}\`${
                        expiry ? ` expiry changed to <t:${expiry}:R>` : ""
                    }${xfold ? ` xFold changed to ${xfold}-fold` : ""}`
                );
                break;
            case "immunity":
                let immunityName = spellcheck
                ...
                if (!immunityId)
                    return await i.editReply({
                        content:
                            "Cannot update an immunity that does not exist, silly",
                    });
                ...
                changes.push(
                    `Changed Immunity: \`${immunityName}\`${
                        expiry ? ` expiry changed to <t:${expiry}:R>` : ""
                    }${xfold ? ` xShot changed to ${xfold}-shot` : ""}`
                );
                break;
            ...
            case "effect":
                ...
                if (!effectId)
                    return await i.editReply({
                        content:
                            "Cannot update an effect that does not exist, silly",
                    });
            ...
            case "ability":
                ...
                if (!abilityId)
                    return await i.editReply({
                        content:
                            "Cannot update an ability that does not exist, silly",
                    });
                ...
                changes.push(
                    `Changed Ability: \`${abilityName}\`${
                        charges ? ` charges changed to ${charges}` : ""
                    }`
                );
                break;
        }
    ...
    try {
        switch (type) {
            case "item":
                ...
                changes.push(
                    `Removed item: \`${itemName}\` (${itemArray.length}/${inventory.inventorySize})`
                );
                break;
            case "aa":
                let anyAbilityName = spellcheck
                    ? await getClosestAbilityName(value)
                    : value;
                let abilityId: number | undefined;
                ...
                if (!abilityId)
                    return await i.editReply({
                        content: "AA does not exist with this name.",
                    });
            ...
            case "status":
                ...
                if (!statusId)
                    return await i.editReply({
                        content:
                            "There is no statuses to remove with this request",
                    });
                ...
                changes.push(
                    `Removed status: \`${statusName}\`${
                        statusExpiry
                            ? ` which would have expired <t:${statusExpiry}:R>`
                            : ""
                    }${statusXFold ? ` which was ${statusXFold}-fold` : ""}`
                );
                break;
            case "immunity":
                let immunityName = spellcheck
                ...
                changes.push(
                    `Removed immunity: \`${immunityName}\`${
                        oldXShot ? ` which was ${oldXShot}-shot` : ""
                    }${
                        oldExpiry
                            ? ` which would have expired <t:${oldExpiry}:R>`
                            : ""
                    }`
                );
                break;
            case "effect":
                ...
                if (!effectId)
                    return await i.editReply({
                        content:
                            "There is no effect to remove with this request",
                    });
                ...
                changes.push(
                    `Removed effect: \`${effectName}\`${
                        effectExpiry
                            ? ` which would have expired <t:${effectExpiry}:R>`
                            : ""
                    }${oldDetails ? ` with details: \`${oldDetails}\`` : ""}`
                );
                break;
            case "perk":
                let perkName = spellcheck
                ...
                if (!perkId)
                    return await i.editReply({
                        content: "There is no perk to remove with this request",
                    });
            ...
            case "ability":
                ...
                if (!baseAbilityId)
                    return await i.editReply({
                        content:
                            "There is no ability to remove with this request",
                    });
