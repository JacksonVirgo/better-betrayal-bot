...
const data = new SlashCommandBuilder().setName('player').setDescription('Manage the players in your game');

data.addSubcommand((sub) =>
	sub
		.setName('list')
		.setDescription('View the list of all players')
		.addBooleanOption((opt) => opt.setName('keep-updated').setDescription('Keep the list updated with new players').setRequired(false))
		.addBooleanOption((opt) => opt.setName('hidden').setDescription('To make this for only you to see').setRequired(false))
);
...
async function showPlayerList(i: ChatInputCommandInteraction) {
	...
	if (!aliveRole || !deadRole) return i.reply({ content: 'Invalid alive or dead role', ephemeral: true });

	let value = `${aliveRole.members.map((v) => `<@&${aliveRole.id}> <@${v.id}>`).join('\n')}`;
	value += `\n${deadRole.members.map((v) => `<@&${deadRole.id}> <@${v.id}>`).join('\n')}`;
