...
const data = new SlashCommandBuilder()
    .setName("post")
    .setDescription("Staff only. Posts important stuff idk");

data.addStringOption((opt) =>
    opt
        .setName("thing")
        .setDescription("The thing to post")
        .setRequired(true)
        .setChoices(
            {
                name: "Item Shop",
...
async function postAAs(i: ChatInputCommandInteraction) {
    ...
    await i.editReply({
        files: [commons, uncommons, rares, epics, legendaries],
    });
}
...
async function getAnyAbilities(rarity: Rarity) {
    ...
    const value =
        `${capitalize(rarity)} AAs\n\n` +
        abilities.join("\n\n") +
        "\n\n" +
        roleSpecific.join("\n");

    const attachment = new AttachmentBuilder(Buffer.from(value)).setName(
        "commons.txt"
    );
