...
data.addSubcommand((sub) =>
	sub
		.setName('custom')
		.setDescription('Submit a custom action, ONLY USE when nothing else works')
		.addStringOption((opt) => opt.setName('actions').setDescription('The action/s to submit').setRequired(true))
		.addIntegerOption((opt) => opt.setName('timestamp').setDescription('FOR AUTOS -> Set a timestamp to perform this action').setRequired(false))
);
...
export default newSlashCommand({
	...
	execute: async (i) => {
		...
		if (!inventory) return i.editReply({ content: 'You need to be a player to use this command' });
