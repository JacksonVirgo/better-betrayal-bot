...
const data = new SlashCommandBuilder()
    .setName("random")
    .setDescription("View commands that give you a random result");
...
data.addSubcommand((sub) =>
    sub
        ...
        .setDescription(
            "Get a random deceptionist pick of 3 roles of each alignment"
        )
        .addStringOption((opt) =>
            opt
                .setName("good-role")
                .setDescription("The good role to use")
                .setRequired(false)
                .setAutocomplete(true)
        )
        .addStringOption((opt) =>
            opt
                .setName("neutral-role")
                .setDescription("The neutral role to use")
                .setRequired(false)
                .setAutocomplete(true)
        )
        .addStringOption((opt) =>
            opt
                .setName("evil-role")
                .setDescription("The evil role to use")
                .setRequired(false)
                .setAutocomplete(true)
        )
        .addBooleanOption((opt) =>
...
data.addSubcommand((sub) =>
    sub
        ...
        .addIntegerOption((opt) =>
            opt
                .setName("players")
                .setDescription("The amount of players")
                .setRequired(true)
        )
        .addIntegerOption((opt) =>
            opt
                .setName("deceptionists")
                .setDescription("The amount of deceptionist picks to get")
                .setRequired(true)
        )
        .addBooleanOption((opt) =>
            opt
...
export default newSlashCommand({
    ...
    autocomplete: async (i) => {
        ...
        switch (subcommand) {
            case "deceptionist":
                let roles =
                    focused.name == "good-role"
                        ? cache.goodRoles
                        : focused.name == "neutral-role"
                        ? cache.neutralRoles
                        : cache.evilRoles;
                let filtered = roles.filter((role) =>
                    role.toLowerCase().startsWith(focused.value.toLowerCase())
                );
                filtered = filtered.splice(0, Math.min(filtered.length, 25));
                return await i.respond(
                    filtered.map((match) => ({ name: match, value: match }))
                );
            default:
                return i.respond([]);
        }
...
async function showRandomRole(i: ChatInputCommandInteraction) {
    ...
    if (!randomRole)
        return i.reply({ content: "No role found", ephemeral: true });
...
async function showDeceptionistPick(i: ChatInputCommandInteraction) {
    ...
    const randomGood = goodRole
        ? await getClosestRoleName(goodRole)
        : (await getRandomRole("GOOD"))?.name;
    const randomNeutral = neutralRole
        ? await getClosestRoleName(neutralRole)
        : (await getRandomRole("NEUTRAL"))?.name;
    const randomEvil = evilRole
        ? await getClosestRoleName(evilRole)
        : (await getRandomRole("EVIL"))?.name;

    if (!(randomGood && randomNeutral && randomEvil))
        return i.reply({
            content: "Failed to fetch one of each alignment",
            ephemeral: true,
        });
    ...
    row.addComponents(
        new ButtonBuilder()
            .setCustomId(viewRole.createCustomID(randomGood))
            .setLabel(randomGood)
            .setStyle(ButtonStyle.Success)
    );
    row.addComponents(
        new ButtonBuilder()
            .setCustomId(viewRole.createCustomID(randomNeutral))
            .setLabel(randomNeutral)
            .setStyle(ButtonStyle.Secondary)
    );
    row.addComponents(
        new ButtonBuilder()
            .setCustomId(viewRole.createCustomID(randomEvil))
            .setLabel(randomEvil)
            .setStyle(ButtonStyle.Danger)
    );

    return i.reply({
        content: "Press the appropriate buttons to see the full role-cards",
        components: [row],
        ephemeral: hidden,
    });
}
...
async function showFullDecept(i: ChatInputCommandInteraction) {
    ...
    const goodAmount = await prisma.role.count({
        where: { alignment: "GOOD", isActive: true },
    });
    const neutralAmount = await prisma.role.count({
        where: { alignment: "NEUTRAL", isActive: true },
    });
    const evilAmount = await prisma.role.count({
        where: { alignment: "EVIL", isActive: true },
    });

    if (decepts > goodAmount && decepts > neutralAmount && decepts > evilAmount)
        return i.reply({
            content: "Not enough roles to generate a setup",
            ephemeral: true,
        });
    ...
    while (goods.length < decepts) {
        ...
        if (!randomGood)
            return i.editReply({ content: "Failed to fetch a good role" });
    ...
    while (neutrals.length < decepts) {
        ...
        if (!randomNeutral)
            return i.editReply({ content: "Failed to fetch a neutral role" });
    ...
    while (evils.length < decepts) {
        ...
        if (!randomEvil)
            return i.editReply({ content: "Failed to fetch a evil role" });
    ...
    while (normalRoles.length < totalNormalPlayers) {
        ...
        if (!randomRole)
            return i.editReply({ content: "Failed to fetch a role" });
    ...
    for (let i = 0; i < totalNormalPlayers; i++) {
        setup.push(
            `${allDeceptionistRoles.includes(normalRoles[i]) ? "- " : ""}${
                normalRoles[i]
            }`
        );
    }
    ...
    if (setup.length < 1)
        return i.editReply({ content: "Failed to generate a setup" });
...
async function showRandomItem(i: ChatInputCommandInteraction) {
    ...
    if (!randomItem)
        return i.reply({ content: "No item found", ephemeral: true });
...
async function showDiceRole(i: ChatInputCommandInteraction) {
    ...
    if (sides < 1)
        return i.reply({
            content: "The amount of sides must be at least 1",
            ephemeral: true,
        });
    if (amount < 1)
        return i.reply({
            content: "The amount of dice must be at least 1",
            ephemeral: true,
        });
