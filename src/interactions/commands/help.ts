...
const topicChoices: APIApplicationCommandOptionChoice<string>[] = topics.map((t) => {
	return { name: capitalize(t), value: t };
});

const data = new SlashCommandBuilder().setName('help').setDescription('Get help on a variety of topics');
...
data.addBooleanOption((opt) => opt.setName('hidden').setDescription('To make this for only you to see'));
